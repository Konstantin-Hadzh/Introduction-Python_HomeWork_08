...
В его конструкторе инициализировать параметр (quantity), соответствующий 
...
class Cell:
    def __init__(self, plenty):
        self.plenty = int(plenty)

    def __add__(self, other):
        return(f'Сумма клеток = ({self.plenty + other.plenty})')
    def __sub__(self, other):
        sub = self.plenty - other.plenty
        return(f'Разность клеток = ({sub})' if sub > 0 else 'Разность отрицательна, поэтому операция не выполняется')

    def __mul__(self, other):
        return(f'Умножение клеток = ({self.plenty * other.plenty})')
    
    def __truediv__(self, other):
        return(f'Деление клеток = ({self.plenty // other.plenty})')
